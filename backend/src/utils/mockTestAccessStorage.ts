...
const ensureProductMockTestTable = async (): Promise<void> => {
  ...
  if (!(await hasColumn("ProductMockTest", "flowType"))) {
    await prisma
      .$executeRawUnsafe("ALTER TABLE `ProductMockTest` ADD COLUMN `flowType` VARCHAR(20) NULL")
      .catch(() => undefined);
  }
...
const ensureProductDemoMockTestTable = async (): Promise<void> => {
  ...
  if (!(await hasColumn("ProductDemoMockTest", "flowType"))) {
    await prisma
      .$executeRawUnsafe("ALTER TABLE `ProductDemoMockTest` ADD COLUMN `flowType` VARCHAR(20) NULL")
      .catch(() => undefined);
  }
