...
const debugPort = Number(process.env.TEACHER_DEBUG_PORT || "9241");
...
(async () => {
  ...
  const sendMessage = async (content) =>
    fetchJson(`http://localhost:5000/student/tuition/chapters/${chapterId}/sessions/${sessionId}/messages`, {
      method: "POST",
      headers,
      body: JSON.stringify({
        content,
  ...
  await sendMessage("__START_TUITION_AI_TEACHER__");
  ...
  const readUiState = async () =>
    evaluate(`(() => {
      const lastTeacher = document.querySelector('.tuition-chat-message.is-assistant:last-of-type');
      const boardFocus = document.querySelector('#tuitionTeacherBoardCurrentConcept');
      const modalCard = document.querySelector('.tuition-teacher-board-card-modal');
      const rectVisible = (el) => {
        if (!el) return false;
        const r = el.getBoundingClientRect();
        const container = modalCard ? modalCard.getBoundingClientRect() : { top: 0, bottom: window.innerHeight };
        return r.top >= container.top && r.bottom <= container.bottom;
      };
      return {
        title: document.querySelector('#tuitionTeacherTitle')?.innerText.trim(),
        teacherIntro: [...document.querySelectorAll('.tuition-chat-message.is-assistant')].at(-1)?.innerText.trim() || '',
        ...
        teacherVisible: rectVisible(lastTeacher),
        boardVisible: rectVisible(boardFocus),
  ...
  const startState = await readUiState();
  ...
  const doubtState = await readUiState();

  await evaluate(`window.scrollTo(0, document.body.scrollHeight); true;`);
  await delay(500);
  const beforeContinue = await readUiState();
  ...
  const resumeState = await readUiState();
  const screenshot = path.join("D:/cc_academy", `browser-focus-${caseName}.png`);
  const shot = await send("Page.captureScreenshot", { format: "png" });
  fs.writeFileSync(screenshot, Buffer.from(shot.data, "base64"));

  console.log(JSON.stringify({ caseName, sessionId, screenshot, startState, doubtState, beforeContinue, resumeState }, null, 2));
  ws.close();
})().catch((error) => {
  console.error(error);
  process.exit(1);
